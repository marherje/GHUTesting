    ...
    for(int iobs=0; iobs<observables.size(); iobs++) {//loop for reference model
        ...
        for(int jobs=0; jobs<observables.size(); jobs++) {
          ...
          if(tpc_status=="noTPC"){
            ...
            //cout<<"debug 1: "<<cross_SM_L / observables.at(jobs).Cross_L<<" "<<cross_SM_R / observables.at(jobs).Cross_R<<endl;
            //cout<<"debug 2: "<<experimental_uncertainties.afb_stat_dNdx[0][index_energy][iquark] / 100.<<" "<<experimental_uncertainties.afb_stat_dNdx[1][index_energy][iquark] / 100.<<endl;
          }
