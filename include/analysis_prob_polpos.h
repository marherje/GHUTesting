...
void read_all_models(bool debug = true)
{
 ...
 std::vector<observables_struct_t> create_observables(float polpos) {
    ...
    for (int imodel = 0; imodel < theory.size(); imodel++)
    {
	  ...
	  for (int iflav = 4; iflav < 6; iflav++)
	      ...
	      newobs.flav = iflav;
	      newobs.Cross_L = ObsCross(imodel, ienergy, iflav, -0.8,polpos);
	      newobs.Cross_R = ObsCross(imodel, ienergy, iflav, 0.8,-1.*polpos);
	      newobs.Cross_unpol = ObsCross(imodel, ienergy, iflav, 0,0);
	      newobs.R_L = ObsR(imodel, ienergy, iflav, -0.8,polpos);
	      newobs.AFB_L = ObsAFB(imodel, ienergy, iflav, -0.8,polpos);
	      newobs.R_R = ObsR(imodel, ienergy, iflav, 0.8,-1.*polpos);
	      newobs.AFB_R = ObsAFB(imodel, ienergy, iflav,0.8,-1.*polpos);
...
std::vector<observables_struct_t> create_observables_nom() {

  std::vector<observables_struct_t> observablesnom;
  for (int imodel = 0; imodel < theory.size(); imodel++)
    {
      for (int ienergy = 0; ienergy < 6; ienergy++) //Z-Pole,250,350,380,500,1000                                                                          
        {
	      ...
	      newobs.R_unpol = ObsR(imodel, ienergy, iflav, 0,0);
	      newobs.AFB_unpol = ObsAFB(imodel, ienergy, iflav, 0,0);
	      observablesnom.push_back(newobs);
            }
        }
    }
  return observablesnom;
}



std::vector<TH2F*> nsigmas_models(int quark_under_test, float energy_under_test, std::vector<observables_struct_t> observablesnew, std::vector<observables_struct_t> observables, TString errortype, TString tpc_status, TString polposstring) {

  
    TH2F * h_nsigmas[8];

    for(int i=0; i<8; i++) {
      h_nsigmas[i]=new TH2F(TString::Format("h_nsigmas_%i_q%i_en%i_",i,quark_under_test,int(energy_under_test))+errortype+"_"+tpc_status+"_"+polposstring,TString::Format("h_nsigmas_%i_q%i_en%i_",i,quark_under_test,int(energy_under_test))+errortype+"_"+tpc_status+"_"+polposstring,31,-0.5,30.5,31,-0.5,30.5);
    }
    ...
    //Alternative method: sqrt(cross_SM_L/cross_SM_L_nom) gives same result
    float cross_SM_L_new;
    float cross_SM_R_new;
    float cross_SM_unpol_new;
    ...
    for(int iobs=0; iobs<observables.size(); iobs++) {//loop for reference model
        ...
        if(id1==0) { //Remember SMB
            cross_SM_L_new=observablesnew.at(iobs).Cross_L;
            cross_SM_R_new=observablesnew.at(iobs).Cross_R;
            cross_SM_unpol_new=observablesnew.at(iobs).Cross_unpol;
	    cross_SM_L=observablesnew.at(iobs).Cross_L;
            cross_SM_R=observablesnew.at(iobs).Cross_R;
            cross_SM_unpol=observablesnew.at(iobs).Cross_unpol;
	}
	    ...
	    //sqrt(observables.at(jobs).Cross_L/observablesnom.at(jobs).Cross_L)*
	    //Total errors:
	    float r_total_L=sqrt(observables.at(jobs).Cross_L/observablesnew.at(jobs).Cross_L)*sqrt(r_stat_L*r_stat_L+r_syst_L*r_syst_L);
            float r_total_R=sqrt(observables.at(jobs).Cross_R/observablesnew.at(jobs).Cross_R)*sqrt(r_stat_R*r_stat_R+r_syst_R*r_syst_R);
            float afb_total_L=sqrt(observables.at(jobs).Cross_L/observablesnew.at(jobs).Cross_L)*sqrt(afb_stat_L*afb_stat_L+afb_syst_L*afb_syst_L+afb_theo_L*afb_theo_L);
            float afb_total_R=sqrt(observables.at(jobs).Cross_R/observablesnew.at(jobs).Cross_R)*sqrt(afb_stat_R*afb_stat_R+afb_syst_R*afb_syst_R+afb_theo_R*afb_theo_R);
            float r_total_unpol=sqrt(observables.at(jobs).Cross_unpol/observablesnew.at(jobs).Cross_unpol)*sqrt(r_stat_unpolarised*r_stat_unpolarised+r_syst_unpolarised*r_syst_unpolarised);
            float afb_total_unpol=sqrt(observables.at(jobs).Cross_unpol/observablesnew.at(jobs).Cross_unpol)*sqrt(afb_stat_unpolarised*afb_stat_unpolarised+afb_syst_unpolarised*afb_syst_unpolarised+afb_theo_unpolarised*afb_theo_unpolarised);
            
	    if(index_energy!=1) afb_total_unpol=sqrt(observables.at(jobs).Cross_L/observablesnew.at(jobs).Cross_L)*sqrt(afb_stat_unpolarised*afb_stat_unpolarised+afb_syst_unpolarised*afb_syst_unpolarised+afb_theo_unpolarised*afb_theo_unpolarised);
	    
	    // nsigmas formula, using uncertainty of the tested model           
	    //P(P, Q) = (1/2πSPSQ) * exp[-((P-P')^2/2SP^2) - ((Q-Q')^2/2SQ^2)];
	    sigmas_AFB_L=abs(observablesnew.at(jobs).AFB_L-observablesnew.at(iobs).AFB_L) / abs(observablesnew.at(iobs).AFB_L*afb_total_L);
	    sigmas_AFB_R=abs(observablesnew.at(jobs).AFB_R-observablesnew.at(iobs).AFB_R) / abs(observablesnew.at(iobs).AFB_R*afb_total_R);
	    sigmas_AFB_unpol=abs(observablesnew.at(jobs).AFB_unpol-observablesnew.at(iobs).AFB_unpol) / abs(observablesnew.at(iobs).AFB_unpol*afb_total_unpol);
	    sigmas_R_L=abs(observablesnew.at(jobs).R_L-observablesnew.at(iobs).R_L) / (observablesnew.at(iobs).R_L*r_total_L);
	    sigmas_R_R=abs(observablesnew.at(jobs).R_R-observablesnew.at(iobs).R_R) / (observablesnew.at(iobs).R_R*r_total_R);
	    sigmas_R_unpol=abs(observablesnew.at(jobs).R_unpol-observablesnew.at(iobs).R_unpol) / (observablesnew.at(iobs).R_unpol*r_total_unpol);
