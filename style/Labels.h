//
//   @file    QQbarLabels.h         
//   
//   @author A. Irles
// 

#ifndef __QQBARLABELS_H
#define __QQBARLABELS_H

#include "Rtypes.h"

void QQBARLabel(Double_t x,Double_t y,TString text="",Color_t color=1);
void QQBARLabel2(Double_t x,Double_t y,TString text="",Color_t color=1, Double_t textsize=0.03, Double_t angle=0);
void QQBARLabel3(Double_t x,Double_t y,TString text="",Color_t color=1, Double_t textsize=0.03);

#endif // __QQBARLABELS_H
